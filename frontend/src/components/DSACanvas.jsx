...
function layoutBST(node, x, y, spread, out = []) {
  ...
  out.push({
    val: node.val, x, y,
    left: node.left ? { x: x - spread, y: y + TREE_DY } : null,
    right: node.right ? { x: x + spread, y: y + TREE_DY } : null,
  });
...
export default function DSACanvas({ socket, roomId }) {
  ...
  // ── Edit state — shown in a toolbar bar, not a floating overlay ──
  const [editCell, setEditCell] = useState(null); // { objId, idx, value, type, label }
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === 'Escape') { setActiveTool(null); setSelectedId(null); setEditCell(null); }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [selectedId, undo, redo, removeObject, setActiveTool, setSelectedId]);

  // ── SVG background click → place or deselect ───────────────
  const onSvgPointerDown = useCallback((e) => {
    const isBg = e.target === svgRef.current || e.target.tagName === 'svg';
    if (!isBg) return;
    if (activeTool) {
  ...
  const startDrag = useCallback((e, id) => {
    ...
    if (!obj) return;
    dragRef.current = { id, startX: e.clientX, startY: e.clientY, origX: obj.x, origY: obj.y };
    didDrag.current = false;
    setSelectedId(id);
  }, [objects, setSelectedId]);
  ...
  const onPointerMove = useCallback((e) => {
    if (dragRef.current) {
      ...
      if (Math.abs(dx) > 3 || Math.abs(dy) > 3) {
        ...
        moveObject(d.id, d.origX + dx, d.origY + dy);
      }
  ...
  // ── Click-to-edit: opens the edit bar at the top ───────────
  const openEdit = useCallback((objId, idx, currentValue, typeLabel) => {
    if (didDrag.current) return;
    setEditCell({ objId, idx, value: String(currentValue), label: typeLabel });
  ...
  // ── Render helpers ─────────────────────────────────────────
  const isSel = (id) => id === selectedId ? 'dsa-selected' : '';

  /* Clickable value text — opens edit bar */
  const ValText = ({ objId, idx, val, x, y, label }) => (
    <text x={x} y={y} className="dsa-val"
      onClick={(e) => { e.stopPropagation(); openEdit(objId, idx, val, label); }}>
      {val}
    </text>
  );
  ...
  function renderArray(obj) {
    ...
    return (
      <g key={obj.id} transform={`translate(${obj.x},${obj.y})`}
         ...
         className={`dsa-obj ${isSel(obj.id)}`}>
        <text x={0} y={-10} className="dsa-type-label">Array</text>
        {values.map((v, i) => (
          <g key={i}>
            <rect x={i * CELL} y={0} width={CELL} height={CELL_H} rx={2} className="dsa-cell" />
            <ValText objId={obj.id} idx={i} val={v} x={i * CELL + CELL / 2} y={CELL_H / 2 + 1} label={`Array[${i}]`} />
            <text x={i * CELL + CELL / 2} y={CELL_H + 14} className="dsa-idx">[{i}]</text>
          </g>
        ))}
        {selectedId === obj.id && (
          <g className="dsa-actions" transform={`translate(${w + 6}, 0)`}>
            <rect x={0} y={0} width={22} height={CELL_H / 2 - 1} rx={3} className="dsa-add-btn"
              onClick={(e) => { e.stopPropagation(); updateObjectData(obj.id, d => ({ ...d, values: [...d.values, 0] })); }} />
            <text x={11} y={CELL_H / 4 + 1} className="dsa-add-label">+</text>
            {values.length > 1 && <>
              <rect x={0} y={CELL_H / 2 + 1} width={22} height={CELL_H / 2 - 1} rx={3} className="dsa-del-btn"
                onClick={(e) => { e.stopPropagation(); updateObjectData(obj.id, d => ({ ...d, values: d.values.slice(0, -1) })); }} />
  ...
  function renderLinkedList(obj) {
    ...
    return (
      <g key={obj.id} transform={`translate(${obj.x},${obj.y})`}
        ...
        {values.map((v, i) => {
          ...
          return (
            <g key={i}>
              ...
              <ValText objId={obj.id} idx={i} val={v} x={nx + nw / 2} y={CELL_H / 2 + 1} label={`Node ${i}`} />
        ...
        {selectedId === obj.id && (
          <g className="dsa-actions" transform={`translate(${values.length * (nw + gap) + 20}, 0)`}>
            <rect x={0} y={0} width={22} height={CELL_H / 2 - 1} rx={3} className="dsa-add-btn"
              onClick={(e) => { e.stopPropagation(); updateObjectData(obj.id, d => ({ ...d, values: [...d.values, 0] })); }} />
            <text x={11} y={CELL_H / 4 + 1} className="dsa-add-label">+</text>
            {values.length > 1 && <>
      ...
      <g key={obj.id} transform={`translate(${obj.x},${obj.y})`}
        ...
        {[...values].reverse().map((v, i) => (
          <g key={i}>
            ...
            <ValText objId={obj.id} idx={values.length - 1 - i} val={v} x={CELL * 0.75} y={i * CELL_H + CELL_H / 2 + 1} label={`Stack[${values.length - 1 - i}]`} />
            {i === 0 && <text x={CELL * 1.5 + 8} y={i * CELL_H + CELL_H / 2 + 1} className="dsa-ptr">← top</text>}
          </g>
        ))}
        {selectedId === obj.id && (
          <g className="dsa-actions" transform={`translate(${CELL * 1.5 + 6}, ${values.length * CELL_H - CELL_H})`}>
            <rect x={0} y={0} width={22} height={CELL_H / 2 - 1} rx={3} className="dsa-add-btn"
              onClick={(e) => { e.stopPropagation(); updateObjectData(obj.id, d => ({ ...d, values: [...d.values, 0] })); }} />
            <text x={11} y={CELL_H / 4 + 1} className="dsa-add-label">+</text>
            {values.length > 1 && <>
      ...
      <g key={obj.id} transform={`translate(${obj.x},${obj.y})`}
        ...
        {values.map((v, i) => (
          <g key={i}>
            ...
            <ValText objId={obj.id} idx={i} val={v} x={i * CELL + CELL / 2} y={CELL_H / 2 + 1} label={`Queue[${i}]`} />
          </g>
        ))}
        ...
        {selectedId === obj.id && (
          <g className="dsa-actions" transform={`translate(${values.length * CELL + 6}, 0)`}>
            <rect x={0} y={0} width={22} height={CELL_H / 2 - 1} rx={3} className="dsa-add-btn"
              onClick={(e) => { e.stopPropagation(); updateObjectData(obj.id, d => ({ ...d, values: [...d.values, 0] })); }} />
            <text x={11} y={CELL_H / 4 + 1} className="dsa-add-label">+</text>
            {values.length > 1 && <>
      ...
      <g key={obj.id} transform={`translate(${obj.x},${obj.y})`}
        ...
        {nodes.map((n, i) => (
          <g key={`n${i}`}>
            ...
            <ValText objId={obj.id} idx={i} val={n.val} x={n.x} y={n.y + 1} label={`BST node`} />
          </g>
        ))}
        {selectedId === obj.id && (
          <g className="dsa-actions" transform={`translate(${90}, ${-NODE_R - 6})`}>
            <rect x={0} y={0} width={22} height={18} rx={3} className="dsa-add-btn"
              onClick={(e) => { e.stopPropagation(); const v = Math.floor(Math.random() * 100); updateObjectData(obj.id, d => ({ ...d, values: [...d.values, v] })); }} />
            <text x={11} y={13} className="dsa-add-label">+</text>
            {obj.data.values.length > 1 && <>
              <rect x={26} y={0} width={22} height={18} rx={3} className="dsa-del-btn"
                onClick={(e) => { e.stopPropagation(); updateObjectData(obj.id, d => ({ ...d, values: d.values.slice(0, -1) })); }} />
    ...
    return (
      <g key={obj.id} transform={`translate(${obj.x},${obj.y})`}
        ...
        {edges.map((ed, i) => {
          ...
          return (
            <g key={i}>
              <line x1={from.rx} y1={from.ry} x2={to.rx} y2={to.ry} className="dsa-edge"
                markerEnd={ed.directed ? 'url(#ah)' : undefined} />
        ...
        {nodes.map((n, i) => (
          <g key={n.id}>
            ...
            <ValText objId={obj.id} idx={i} val={n.label} x={n.rx} y={n.ry + 1} label={`Node ${n.label}`} />
          </g>
        ))}
        {selectedId === obj.id && (
          <g className="dsa-actions" transform={`translate(${130}, ${-NODE_R - 6})`}>
            <rect x={0} y={0} width={22} height={18} rx={3} className="dsa-add-btn"
              onClick={(e) => { e.stopPropagation(); updateObjectData(obj.id, d => { const id = `n${Date.now()}`; return { ...d, nodes: [...d.nodes, { id, label: String.fromCharCode(65 + d.nodes.length), rx: 60, ry: 50 + d.nodes.length * 30 }] }; }); }} />
            <text x={11} y={13} className="dsa-add-label">+</text>
          </g>
        )}
      </g>
  ...
  function renderLabel(obj) {
    return (
      <g key={obj.id} transform={`translate(${obj.x},${obj.y})`}
         onPointerDown={(e) => startDrag(e, obj.id)}
         className={`dsa-obj ${isSel(obj.id)}`}>
        <text x={0} y={0} className="dsa-label-text" style={{ fontSize: obj.data.fontSize }}
          onClick={(e) => { e.stopPropagation(); openEdit(obj.id, 0, obj.data.text, 'Label text'); }}>
          {obj.data.text}
        </text>
      </g>
    );
  ...
  return (
    <div className="dsa-container">
      ...
      <div className="dsa-toolbar">
        {DSA_TOOLS.map(t => (
          <button key={t.type}
            className={`dsa-tool-btn ${activeTool === t.type ? 'dsa-tool-btn--active' : ''}`}
        ...
        {selectedId && <>
          <span className="dsa-tb-div" />
          <button className="dsa-tool-btn dsa-tool-btn--danger"
            onClick={() => removeObject(selectedId)} title="Delete (Del)">✕ Delete</button>
        </>}
        {activeTool && <span className="dsa-place-hint">Click on canvas to place {activeTool}</span>}
      </div>

      {/* Edit bar — appears when editing a value */}
      {editCell && (
      ...
      <div className="dsa-viewport">
        <svg ref={svgRef} className="dsa-svg"
